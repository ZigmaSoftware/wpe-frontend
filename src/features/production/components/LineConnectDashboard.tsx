import { CalendarDays, ChevronDown, Circle, Clock3, Link2, Link2Off, type LucideIcon } from "lucide-react";
...
> = {
  connected: {
    ...
    hoverCardClassName: "hover:-translate-y-1 hover:border-emerald-400 hover:shadow-[0_14px_30px_rgba(16,185,129,0.18)]",
  ...
  not_connected: {
    ...
    hoverCardClassName: "hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_12px_24px_rgba(15,23,42,0.08)]",
  ...
  return (
    <section className="rounded-[22px] border border-slate-200/80 bg-white px-4 py-4 shadow-sm sm:px-5">
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        <CollapsibleTrigger
          aria-label={isOpen ? "Collapse line connection status" : "Expand line connection status"}
          className="inline-flex items-center gap-2 text-left text-[15px] font-bold uppercase tracking-[0.08em] text-slate-950 transition-colors duration-200 hover:text-slate-700 [&[data-state=open]>svg]:rotate-180"
        >
          <span>Line Connection Status</span>
          <ChevronDown className="h-4 w-4 shrink-0 transition-transform duration-300 ease-out" />
        </CollapsibleTrigger>

        <CollapsibleContent className="group/collapsible overflow-hidden will-change-[height] data-[state=closed]:animate-[accordion-up_340ms_cubic-bezier(0.4,0,0.2,1)] data-[state=open]:animate-[accordion-down_420ms_cubic-bezier(0.22,1,0.36,1)] motion-reduce:animate-none">
          <div className="pt-3 transition-[opacity,transform] duration-300 ease-out group-data-[state=closed]/collapsible:-translate-y-0.5 group-data-[state=closed]/collapsible:opacity-0 group-data-[state=open]/collapsible:translate-y-0 group-data-[state=open]/collapsible:opacity-100 motion-reduce:transition-none">
                    ...
                    {dashboardItems.map(({ line, connection, state }) => {
                      ...
                      return (
                        ...
                        >
                          <article
                            className={cn(
                              "group relative flex h-full flex-1 cursor-default flex-col rounded-[18px] border bg-white px-3.5 py-3.5 shadow-[0_5px_16px_rgba(15,23,42,0.04)] transition-all duration-200 ease-out",
                              meta.borderClassName,
                              meta.hoverCardClassName,
                            )}
                          ...
                          >
                            <div className="flex items-start justify-center">
                              <div
                                className={cn(
                                  "flex h-8 w-8 items-center justify-center rounded-full border-[3px] border-white transition-transform duration-200 group-hover:scale-105",
                                  meta.iconClassName,
                                )}
                              >
