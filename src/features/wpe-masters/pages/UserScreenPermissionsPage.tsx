...
import type { UserScreenPermRow, UserScreenPermKey } from "@/features/wpe-masters/types";
import { CheckSquare, Save, Monitor } from "lucide-react";

/* ─── column definitions ─────────────────────────────────────────── */
type ColType = "radio-all" | "radio-self" | "checkbox" | "toggle";

const COLUMNS: { key: UserScreenPermKey; label: string; short: string; type: ColType }[] = [
...
const CHECKBOX_KEYS: UserScreenPermKey[] = ["can_add", "can_edit", "can_duplicate", "can_delete"];
const TOGGLE_KEYS:   UserScreenPermKey[] = ["generate_invoice_access", "invoice_access", "access"];
const ALL_BOOL_KEYS: UserScreenPermKey[] = [...CHECKBOX_KEYS, ...TOGGLE_KEYS];
...
const UserScreenPermissionsPage = () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["wpe-user-screen-perm", "matrix", activeScreen] });
      setDirty(false);
    },
  ...
  /* mutations */
  const patch = (screenId: number, updates: Partial<UserScreenPermRow>) => {
    setMatrix((prev) => prev.map((r) => r.user_screen_id === screenId ? { ...r, ...updates } : r));
    setDirty(true);
  };

  const setView = (screenId: number, mode: "all" | "self" | "none") =>
    patch(screenId, { view_all: mode === "all", view_self: mode === "self" });

  const toggleField = (screenId: number, key: UserScreenPermKey) => {
    const row = matrix.find((r) => r.user_screen_id === screenId);
    if (row) patch(screenId, { [key]: !row[key] });
  };

  /* column-header "select all" */
  const allChecked = (key: UserScreenPermKey) => matrix.length > 0 && matrix.every((r) => r[key]);

  const toggleCol = (key: UserScreenPermKey) => {
  ...
  const toggleRow = (screenId: number) => {
    const row = matrix.find((r) => r.user_screen_id === screenId);
    if (!row) return;
        ...
        <>
          ...
          <div className="relative mt-4 flex-1 overflow-auto rounded-xl border border-slate-200 shadow-sm">
            {isFetching && matrix.length === 0 ? (
              ...
              <div className="flex items-center justify-center py-20 text-sm text-slate-400">
                No active user screens found for this module. Add screens in Admin → User Screens.
              </div>
            ) : (
              <table className="w-full border-collapse text-sm">
                ...
                <tbody>
                  {matrix.map((row, idx) => (
                    <tr
                      key={row.user_screen_id}
                      className={idx % 2 === 0 ? "bg-white" : "bg-slate-50/60"}
                    >
                      {/* Screen name + section + row toggle */}
                      <td className="sticky left-0 z-10 border-b border-r border-slate-200 bg-inherit px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          <Tick
                            checked={rowAllChecked(row)}
                            onChange={() => toggleRow(row.user_screen_id)}
                          />
                          <div className="min-w-0">
                            <div className="font-medium text-slate-700 truncate">{row.screen_name}</div>
                            {row.screen_section_name && (
                              <div className="text-[11px] text-slate-400 truncate">{row.screen_section_name}</div>
                            )}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="border-b border-r border-slate-200 px-3 py-3 text-center">
                        <div className="flex flex-col items-center gap-0.5">
                          <Radio
                            ...
                            onChange={() => setView(row.user_screen_id, row.view_all ? "none" : "all")}
                          />
                      ...
                      <td className="border-b border-r border-slate-200 px-3 py-3 text-center">
                        <div className="flex flex-col items-center gap-0.5">
                          <Radio
                            ...
                            onChange={() => setView(row.user_screen_id, row.view_self ? "none" : "self")}
                          />
                        ...
                        <td key={key} className="border-b border-r border-slate-200 px-3 py-3 text-center">
                          <div className="flex flex-col items-center gap-0.5">
                            <Tick
                              ...
                              onChange={() => toggleField(row.user_screen_id, key)}
                            />
