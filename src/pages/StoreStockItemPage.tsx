...
const StoreStockItemPage = () => {
  ...
  const historyQuery = useQuery({
    ...
    queryKey: ["store", "stock-item-history", numericItemId, filters, deferredSearch],
    queryFn: () =>
      storeApi.listStockItemHistory({
  ...
  const filteredRows = (historyQuery.data ?? []).filter((row) => {
    ...
    if (filters.type === "inwards") return row.transaction_type === "INWARD";
  ...
  const handleExport = (formatType: StoreExportFormat) => {
    try {
      ...
      exportTableData({
        title: `Store Stock History - ${itemRow?.item_name ?? `Item ${numericItemId}`}`,
        filename: `store-stock-history-${numericItemId}`,
        rows: filteredRows,
        columns,
        format: formatType,
      });
  ...
  if (!isValidItemId) {
    return <ErrorState description="Invalid store item. Open this page from Store Stock rows." />;
  }

  return (
    <div className="space-y-3">
      <PageHeader
        title={itemRow?.item_name ? `${itemRow.item_name} History` : "Store Stock Item History"}
        description={itemRow?.item_code ? `Movement ledger for ${itemRow.item_code}.` : "Movement ledger for the selected stock item."}
        actions={
          <Button asChild variant="outline">
            <Link to="/app/store">Back to Store</Link>
          </Button>
        }
      />
          ...
          filterContent={
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1fr_1fr_200px_auto]">
              <div className="space-y-1">
                <label htmlFor="stock-history-from-date" className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
                  From Date
                </label>
                <Input
              ...
              <div className="space-y-1">
                <label htmlFor="stock-history-to-date" className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
                  To Date
                </label>
                <Input
